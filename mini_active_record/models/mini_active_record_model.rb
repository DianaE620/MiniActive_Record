module MiniActiveRecord
  ...
  class Model
    ...
    def self.all
      MiniActiveRecord::Model.execute("SELECT * FROM #{self.table}").map do |row|
        self.new(row)
      end
    end
    ...
    MiniActiveRecord::Model.execute("SELECT * FROM #{self.table} WHERE #{query}", *args).map do |row|
      self.new(row)
    end
  end
  ...
  def insert!
    ...
    insert_sql = "INSERT INTO #{self.class.table} (#{fields.join(',')}) VALUES (#{marks})"
  ...
  def update!
    ...
    update_sql = "UPDATE #{self.class.table} SET #{update_clause} WHERE id = ?"
